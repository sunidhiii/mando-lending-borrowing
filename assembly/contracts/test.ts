import { Address, Context, generateEvent, Storage, createEvent, callerHasWriteAccess } from '@massalabs/massa-as-sdk';
...
export function mintOnDeposit(binaryArgs: StaticArray<u8>): void {
  ...
  generateEvent(`Balance increased after mint ${amount} tokens`)
...
export function burnOnLiquidation(binaryArgs: StaticArray<u8>): void {
  ...
  generateEvent(`Balance decreased after mint from ${user} tokens to ${amount} tokens`)

}

export function burnOnLiquidation1(binaryArgs: StaticArray<u8>): void {
    const args = new Args(binaryArgs);
    const user = new Address(args.nextString().expect('user argument is missing or invalid'));
    const amount =
      args.nextU64().expect('burnOnLiquidation amount argument is missing or invalid');
    ...
    //burns the requested amount of tokens
    burn(new Args().add(u256.fromU64(amount)).serialize());
