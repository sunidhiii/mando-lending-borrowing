...
export class ILendingCore {
  ...
  getUserReserve(_user: Address, _reserve: string): UserReserve {
    const result = call(this._origin, "getUserReserve", new Args().add(_user.toString()).add(_reserve), 0);
  ...
  updateStateOnDeposit(reserve: string, user: string, amount: u64, isFirstDeposit: bool): void {
    call(this._origin, "updateStateOnDeposit", new Args().add(reserve).add(user).add(amount).add(isFirstDeposit), 1);
  }
