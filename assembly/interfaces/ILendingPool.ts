...
export class ILendingPool {
  ...
  deposit(reserve: string, user: string, amount: u64): void {
    call(
      this._origin,
      'deposit',
      new Args().add(reserve).add(user).add(amount),
      0,
    );
  }
  ...
  depositRewards(
    ...
    call(
      this._origin,
      'depositRewards',
      new Args().add(currentReserve).add(reserve).add(user).add(amount),
      2,
    );
  }
